#!/usr/bin/python
from pylab import *
from readfile import *
from mycolor import *

rc('font', size=21)

path     = "../data/params-lsh/params-lsh--geo-W.txt"

label,mat = file2matrix(path,6,';')
rc('font', size=28)
rc('legend', fontsize=21)
#-------------------------------------------------------------------------------

fig =figure(figsize=(16, 8), dpi=80)

#title("LSH : impact W on accuracy\n")
fig.subplots_adjust(bottom=0.1, left=0.1, top = 0.9, right=0.9)
fig.subplots_adjust(hspace=0.5)

taillebar=1.5
ax = fig.add_subplot(1,1,1)
ax.plot(mat[:,0]/10000,mat[:,3]/1000/60, c=colors[1],marker=mark[1],label="time")
#a = ax.bar(mat[:,0]/10000-taillebar/2.0,mat[:,1],taillebar,label="time",color=colors[1],alpha=0.5)
	
ax2 = ax.twinx()
ax2.plot(mat[:,0]/10000,mat[:,1],'--', c=colors[0],marker=mark[0],label="accuracy")
ax.plot(0,0,'--', c=colors[0],marker=mark[0],label="accuracy")

ax.legend(loc='center right',fancybox=True, shadow=True)
ax.set_xlabel('W ($*10^{4}$)')
ax.set_ylabel('Time (minutes)', color=colors[1])
for tl in ax.get_yticklabels():
	    tl.set_color(colors[1])

ax2.set_ylabel('Accuracy')
#ax.set_title("Time vs Accuracy")

bbox_props = dict(boxstyle="rarrow,pad=0.2", fc="pink", ec="k", lw=2)
# t = ax2.text(275000, 0.91, "Optimal Accuracy", ha="center", va="center", rotation=60,
#             size=15,
#             bbox=bbox_props)
grid(True)
show()

#-------------------------------------------------------------------------------

fig2 =figure(figsize=(16, 8), dpi=80)
ax3 = fig2.add_subplot(1,1,1)
ax3.bar(mat[:,0]/10000-1.5,mat[:,5], 3,label="buckets")
ax4 = ax3.twinx()
ax4.plot(mat[:,0]/10000,mat[:,1],'--', c=colors[0],marker=mark[0],label="accuracy")

ax3.plot(0,0,'--', c=colors[0],marker=mark[0],label="accuracy")

#ax3.set_title("#Bucket vs Accuracy")

ax3.legend(loc='center right',fancybox=True, shadow=True)

ax3.set_xlabel('W ($*10^{4}$)')
ax3.set_ylabel('Number of buckets')
ax4.set_ylabel('Accuracy')
grid(True)
#-----------------------------------------------

show()
fig.savefig('../../img-perf/perso/lsh/params_geo_time.pdf',dpi=400)
fig2.savefig('../../img-perf/perso/lsh/params_geo_acc.pdf',dpi=400)



