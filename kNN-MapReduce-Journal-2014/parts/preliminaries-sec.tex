...
%practice, the map and reduce functions can be executed on many machines, leading to a naturally distributed computation. 
...
%\begin{figure}[htp]
%\centering
%\includegraphics[scale=0.23]{res/mapreduce2.png}
%
%\caption{Map Reduce Paradigm}
%\label{mapreduce}
%\end{figure}

%\TODOREP{J'ai mis car je pensais qu 'il voulait qu on detaille une methode mais en fait j crois que c'est une application dc à enlever.The Figure \ref{mapreduce} shows the paradigm of map reduce of first phase of one algorithm hbnlj. Input is splitted in block to map each data with a key $raw,col$ . For the dataset $R$, $raw$ is chosen randomly between 0 and p (here $p=2$) and the data are mapped  $p$ times with $col$ of 0 to p. With the dataset $S$ it's reverse  : the raw that is chosen randomly. Thus,  every pairs r and s  are repartitioned in $p*p reducer$, we apply the $knn$ algorithm  on each reducer to get the $knn$ candidates..}
...
%For instance, LSH-based techniques usually provide many space hashing functions. The space filling curve method also usually needs several shifts 
